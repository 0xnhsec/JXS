...
scope_config = {
    "scope_name": str,
    "host_whitelist": [str],       # daftar host yang boleh di-capture, mis. ["target.com", "www.target.com"]
    "auth_cookie": str | None,     # opsional, dipakai Playwright buat crawl authenticated state
    "host_list_file": str | None,  # opsional, path ke file host list ala JSFinder -c mode
}
```

Field `auth_cookie` dan `host_list_file` opsional — default `None` buat unauthenticated crawl biasa. Kalau target butuh crawl dengan session aktif (misal nemu JS yang cuma load setelah login), isi `auth_cookie` dan Playwright inject sebagai cookie header sebelum `page.goto()`.
...
| Skenario | Handling |
|---|---|
| JS file > 10MB (vendor bundle raksasa) | Skip extraction sepenuhnya, cuma simpan metadata (URL, size, hash), flag sebagai `type: oversized_skipped` |
...
## 8k. Pending Validation (blocker — bukan catatan lepas)
...
**Gate sebelum implementasi:** test pattern terhadap `data/test-fixtures/` dan minimal 2 scope aktif. Target FP rate: HIGH pattern < 10%, MEDIUM < 40%.
...
- **8u** — Validation Checklist ada di PRD sebagai teks (section 8u), tapi belum ada implementasi UI-nya (modal / sidebar checklist interaktif). Saat ini dilakukan manual mengacu ke teks di atas.
...
- **8w** — Insecure Storage Detection. Backlog, unblocked, belum diprioritaskan. Draft pattern sudah ada di section 8w.
